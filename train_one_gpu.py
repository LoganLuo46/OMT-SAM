...
# Optionally, extract the starting epoch if available
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        # 使用tqdm创建进度条
        pbar = tqdm(total=total_batches, desc=f"Training Epoch {epoch + 1}")
        ...
        with torch.no_grad():
            # 计算验证集总批次数
            total_val_batches = sum(len(dl["val"]) for dl in loaders.values())
            print(f"\nValidation - Total batches: {total_val_batches}")
            val_pbar = tqdm(total=total_val_batches, desc="Validating")
            ...
            for oid, dl in loaders.items():
                ...
                for image, gt, boxes, _, text_input in val_loader:
                    bboxes = boxes.numpy()
                ...
                print(
                    f"{organ.capitalize()} Validation Complete: Loss: {val_results[organ]['loss']:.4f}, Dice: {val_results[organ]['dice']:.4f}")
